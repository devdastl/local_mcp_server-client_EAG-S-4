...
In the above tools description, You also have access to four **special** tools which will allow you to interact with pinta application where you can select specified tools and perform specified action. Use this **special** tools only when user ask to doing something with **pinta** application. And make sure that you are following **correct** order of tool call for interacting with pinta application. for example always open pinta application first before doing any thing, then select proper tool for a given action and then lastly perform that action.

Some tools return python dict, be sure to properly parse the dict to get the required values and be mindful that some of the dicts are nested with proper key name
